...
function parseWithCheerio($) {
  ...
  const title = normalizeText($("title").first().text()) || "Almaty people";
  ...
  $("div, article, section, li").each((_, element) => {
    ...
    if (
      text.length >= 40 &&
      text.length <= 900 &&
      /phone|e-mail|email|reception|chief|akim|deputy|mayor/i.test(text)
    ) {
      cards.push(text);
    }
  });
  ...
  return {
    ...
    links,
    people: unique(cards).slice(0, 40),
  };
}
